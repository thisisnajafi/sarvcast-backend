...
deploy_to_ftp() {
    ...
    log_warning "Remember to run these commands on your server via SSH:"
    ...
    log_warning "3. php artisan storage:link --force"
    log_warning "4. php artisan config:cache && php artisan route:cache && php artisan view:cache"
    log_warning "5. chmod -R 755 storage bootstrap/cache"
    log_warning ""
    log_warning "Or run the ensure-storage-link.sh script:"
    log_warning "   bash ensure-storage-link.sh"
}
