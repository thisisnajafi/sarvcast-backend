    ...
    <div class="bg-white shadow-sm rounded-lg">
        ...
        <form method="POST" action="{{ route('admin.influencers.update', $influencer) }}" enctype="multipart/form-data" class="p-6 space-y-6">
            ...
            <div>
                ...
                <select name="user_id" id="user_id" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent @error('user_id') border-red-500 @enderror">
                    <option value="">انتخاب کاربر</option>
                    @foreach($users as $user)
                        <option value="{{ $user->id }}" {{ (old('user_id', $influencer->user_id) == $user->id) ? 'selected' : '' }}>
