...
<div class="space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            ...
            <div id="revenueTrendsChart" class="h-64"></div>
        </div>

        <!-- Payment Methods Chart -->
        <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900 mb-4">روش‌های پرداخت</h3>
            <div id="paymentMethodsChart" class="h-64"></div>
        </div>
    </div>

    <!-- Monthly Comparison -->
    <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">مقایسه ماهانه</h3>
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div class="text-center p-4 bg-gray-50 rounded-lg">
                <div class="text-2xl font-bold text-gray-900">{{ number_format($monthlyComparison['current_month']) }} تومان</div>
                <div class="text-sm text-gray-600">ماه جاری</div>
            </div>
            <div class="text-center p-4 bg-gray-50 rounded-lg">
                <div class="text-2xl font-bold text-gray-900">{{ number_format($monthlyComparison['previous_month']) }} تومان</div>
                <div class="text-sm text-gray-600">ماه گذشته</div>
            </div>
            <div class="text-center p-4 bg-gray-50 rounded-lg">
                <div class="text-2xl font-bold {{ $monthlyComparison['growth_direction'] == 'up' ? 'text-green-600' : 'text-red-600' }}">
                    {{ $monthlyComparison['growth_rate'] > 0 ? '+' : '' }}{{ $monthlyComparison['growth_rate'] }}%
                </div>
                <div class="text-sm text-gray-600">نرخ رشد</div>
            </div>
        </div>
    </div>
    ...
    <!-- Conversion Metrics -->
    <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
        <h3 class="text-lg font-semibold text-gray-900 mb-6">معیارهای تبدیل</h3>
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div class="text-center">
                <div class="text-3xl font-bold text-blue-600">{{ $conversionMetrics['conversion_rate'] }}%</div>
                <div class="text-sm text-gray-600">نرخ تبدیل</div>
            </div>
            <div class="text-center">
                <div class="text-3xl font-bold text-green-600">{{ number_format($conversionMetrics['average_order_value']) }} تومان</div>
                <div class="text-sm text-gray-600">میانگین ارزش سفارش</div>
            </div>
            <div class="text-center">
                <div class="text-3xl font-bold text-purple-600">{{ number_format($conversionMetrics['customer_lifetime_value']) }} تومان</div>
                <div class="text-sm text-gray-600">ارزش طول عمر مشتری</div>
            </div>
            <div class="text-center">
                <div class="text-3xl font-bold text-red-600">{{ $conversionMetrics['churn_rate'] }}%</div>
                <div class="text-sm text-gray-600">نرخ ریزش</div>
            </div>
        </div>
    </div>
