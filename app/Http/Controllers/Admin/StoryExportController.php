...
class StoryExportController extends Controller
{
    ...
    public function exportJson(Request $request)
    {
        ...
        $payload = $this->buildExportPayload($stories);
    ...
    public function importJson(Request $request)
    {
        ...
        try {
            $this->importStoriesFromPayload($data);
    ...
    public function apiExportJson(Request $request): JsonResponse
    {
        $request->validate([
            'story_ids' => 'nullable|array',
            'story_ids.*' => 'integer|exists:stories,id',
    ...
    public function apiImportJson(Request $request): JsonResponse
    {
        $request->validate([
            'file' => 'required|file|mimes:json,txt',
        ]);
        ...
        if (! is_array($data) || ! isset($data['stories']) || ! is_array($data['stories'])) {
            return response()->json([
                'success' => false,
                'message' => 'ساختار فایل JSON نامعتبر است.',
            ], 422);
        }

        try {
            $imported = $this->importStoriesFromPayload($data);

            return response()->json([
                'success' => true,
                'message' => 'داستان‌ها و اپیزودها با موفقیت از فایل JSON وارد شدند.',
                'data' => ['imported_stories' => $imported],
            ]);
        } catch (\Exception $e) {
            Log::error('Story JSON API import failed', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString(),
            ]);
    ...
    private function buildExportPayload($stories): array
    {
        $payload = [
            ...
            $payload['stories'][] = [
                ...
                'episodes' => $story->episodes->map(function (Episode $episode) {
                    return [
                        ...
                        'image_timelines' => $episode->imageTimelines->map(fn (ImageTimeline $timeline) => $timeline->toArray())->all(),
    ...
    private function importStoriesFromPayload(array $data): int
    {
        DB::beginTransaction();
